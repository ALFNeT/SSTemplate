--Buffer pool - empty space
SELECT  (CASE WHEN ([database_id] = 32767) THEN N'Resource Database'
...
--Buffer pool empty space by DB and object
EXEC sp_MSforeachdb N'
    IF EXISTS (SELECT 1 FROM (SELECT DISTINCT DB_NAME ([database_id]) AS [name]
...
BEGIN
	...
	FROM    (SELECT allocation_unit_id
				   ...
				   ,SUM(CASE WHEN ([is_modified] = 1) THEN 0
							 ELSE 1
						END) AS [CPCount]
				   ,SUM(CASE WHEN ([is_modified] = 1) THEN CAST ([free_space_in_bytes] AS BIGINT)
							 ELSE 0
						END) AS [DPFreeSpace]
				   ,SUM(CASE WHEN ([is_modified] = 1) THEN 0
