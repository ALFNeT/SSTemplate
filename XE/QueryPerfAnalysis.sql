...
ADD EVENT sqlserver.sql_statement_completed (
    ...
    WHERE ( [package0].[greater_than_uint64]([sqlserver].[database_id], ( 4 ))
            AND [package0].[equal_boolean]([sqlserver].[is_system], ( 0 )) ) )
ADD TARGET package0.event_file (  SET FILENAME = N'<path>.xel' ,
                                  max_file_size = ( 64 ) ,
...
IF OBJECT_ID('tempdb..#xevents') IS NULL
...
IF OBJECT_ID('tempdb..waits') IS NULL
    CREATE TABLE #waits
        (
          event_time DATETIME2 ,
...
IF OBJECT_ID('tempdb..query_stats') IS NULL
...
INSERT  INTO #xevents
        ...
        SELECT  DATEADD(mi, DATEDIFF(mi, GETUTCDATE(), CURRENT_TIMESTAMP),
                ...
                x.event_data.value('(event/action[@name="query_hash"])[1]',
                                   'decimal(38,0)') ,
                x.event_data
        FROM    sys.fn_xe_file_target_read_file('C:\temp\query_performance*.xel',
                                                'C:\temp\query_performance*.xem',
                                                NULL, NULL)
...
INSERT  INTO #waits
        SELECT  DATEADD(mi, DATEDIFF(mi, GETUTCDATE(), CURRENT_TIMESTAMP),
                ...
                x.event_data.value('(event/action[@name="query_hash"])[1]',
                                   'decimal(38,0)') AS query_hash ,
                x.event_data.value('(event/action[@name="query_plan_hash"])[1]',
                                   'decimal(38,0)') AS query_plan_hash ,
                x.event_data.value('(event/action[@name="session_id"])[1]',
                                   'int') AS session_id ,
                x.event_data.value('(event/action[@name="client_hostname"])[1]',
                                   'nvarchar(max)') AS client_hostname ,
                x.event_data.value('(event/action[@name="database_name"])[1]',
                                   'nvarchar(max)') AS database_name ,
                x.event_data.value('(event/action[@name="sql_text"])[1]',
                                   'nvarchar(max)') AS statement ,
                x.event_data.value('(event/data[@name="wait_type"]/text)[1]',
                                   'nvarchar(max)') AS wait_type ,
                x.event_data.value('(event/data[@name="duration"])[1]', 'int') AS duration_ms ,
                x.event_data.value('(event/data[@name="signal_duration"])[1]',
                                   'int') AS signal_duration_ms
        FROM    #xevents AS x
...
INSERT  INTO #query_stats
        SELECT  DATEADD(mi, DATEDIFF(mi, GETUTCDATE(), CURRENT_TIMESTAMP),
                ...
                x.event_data.value('(event/action[@name="database_name"])[1]',
                                   'nvarchar(max)') AS database_name ,
                x.event_data.value('(event/data[@name="statement"])[1]',
                                   'nvarchar(max)') AS statement ,
                x.event_data.value('(event/data[@name="duration"])[1]', 'int') AS duration_ms ,
                x.event_data.value('(event/data[@name="cpu_time"])[1]', 'int') AS cpu_time_ms ,
                x.event_data.value('(event/data[@name="physical_reads"])[1]',
                                   'int') AS physical_reads ,
                x.event_data.value('(event/data[@name="logical_reads"])[1]',
                                   'int') AS logical_reads ,
...
WITH    statement_ntile_cte
          AS ( SELECT  DISTINCT
                        event_interval ,
                        query_hash ,
                        query_plan_hash ,
                        PERCENTILE_CONT(0.50) WITHIN GROUP ( ORDER BY duration_ms ) OVER ( PARTITION BY query_plan_hash,
                                                              DATEADD(MINUTE,
                                                              DATEDIFF(MINUTE,
                                                              0, event_time),
                                                              0) ) AS duration_50th ,
                        PERCENTILE_CONT(0.75) WITHIN GROUP ( ORDER BY duration_ms ) OVER ( PARTITION BY query_plan_hash,
                                                              DATEADD(MINUTE,
                                                              DATEDIFF(MINUTE,
                                                              0, event_time),
                                                              0) ) AS duration_75th ,
                        PERCENTILE_CONT(0.90) WITHIN GROUP ( ORDER BY duration_ms ) OVER ( PARTITION BY query_plan_hash,
                                                              DATEADD(MINUTE,
                                                              DATEDIFF(MINUTE,
                                                              0, event_time),
                                                              0) ) AS duration_90th ,
                        PERCENTILE_CONT(0.95) WITHIN GROUP ( ORDER BY duration_ms ) OVER ( PARTITION BY query_plan_hash,
                                                              DATEADD(MINUTE,
                                                              DATEDIFF(MINUTE,
                                                              0, event_time),
                                                              0) ) AS duration_95th ,
                        PERCENTILE_CONT(0.99) WITHIN GROUP ( ORDER BY duration_ms ) OVER ( PARTITION BY query_plan_hash,
                                                              DATEADD(MINUTE,
                                                              DATEDIFF(MINUTE,
                                                              0, event_time),
                                                              0) ) AS duration_99th ,
                        PERCENTILE_CONT(0.50) WITHIN GROUP ( ORDER BY cpu_time_ms ) OVER ( PARTITION BY query_plan_hash,
                                                              DATEADD(MINUTE,
                                                              DATEDIFF(MINUTE,
                                                              0, event_time),
                                                              0) ) AS cpu_time_50th ,
                        PERCENTILE_CONT(0.75) WITHIN GROUP ( ORDER BY cpu_time_ms ) OVER ( PARTITION BY query_plan_hash,
                                                              DATEADD(MINUTE,
                                                              DATEDIFF(MINUTE,
                                                              0, event_time),
                                                              0) ) AS cpu_time_75th ,
                        PERCENTILE_CONT(0.90) WITHIN GROUP ( ORDER BY cpu_time_ms ) OVER ( PARTITION BY query_plan_hash,
                                                              DATEADD(MINUTE,
                                                              DATEDIFF(MINUTE,
                                                              0, event_time),
                                                              0) ) AS cpu_time_90th ,
                        PERCENTILE_CONT(0.95) WITHIN GROUP ( ORDER BY cpu_time_ms ) OVER ( PARTITION BY query_plan_hash,
                                                              DATEADD(MINUTE,
                                                              DATEDIFF(MINUTE,
                                                              0, event_time),
                                                              0) ) AS cpu_time_95th ,
                        PERCENTILE_CONT(0.99) WITHIN GROUP ( ORDER BY cpu_time_ms ) OVER ( PARTITION BY query_plan_hash,
                                                              DATEADD(MINUTE,
                                                              DATEDIFF(MINUTE,
                                                              0, event_time),
                                                              0) ) AS cpu_time_99th ,
                        PERCENTILE_CONT(0.50) WITHIN GROUP ( ORDER BY physical_reads ) OVER ( PARTITION BY query_plan_hash,
                                                              DATEADD(MINUTE,
                                                              DATEDIFF(MINUTE,
                                                              0, event_time),
                                                              0) ) AS physical_reads_50th ,
                        PERCENTILE_CONT(0.75) WITHIN GROUP ( ORDER BY physical_reads ) OVER ( PARTITION BY query_plan_hash,
                                                              DATEADD(MINUTE,
                                                              DATEDIFF(MINUTE,
                                                              0, event_time),
                                                              0) ) AS physical_reads_75th ,
                        PERCENTILE_CONT(0.90) WITHIN GROUP ( ORDER BY physical_reads ) OVER ( PARTITION BY query_plan_hash,
                                                              DATEADD(MINUTE,
                                                              DATEDIFF(MINUTE,
                                                              0, event_time),
                                                              0) ) AS physical_reads_90th ,
                        PERCENTILE_CONT(0.95) WITHIN GROUP ( ORDER BY physical_reads ) OVER ( PARTITION BY query_plan_hash,
                                                              DATEADD(MINUTE,
                                                              DATEDIFF(MINUTE,
                                                              0, event_time),
                                                              0) ) AS physical_reads_95th ,
                        PERCENTILE_CONT(0.99) WITHIN GROUP ( ORDER BY physical_reads ) OVER ( PARTITION BY query_plan_hash,
                                                              DATEADD(MINUTE,
                                                              DATEDIFF(MINUTE,
                                                              0, event_time),
                                                              0) ) AS physical_reads_99th ,
                        PERCENTILE_CONT(0.50) WITHIN GROUP ( ORDER BY logical_reads ) OVER ( PARTITION BY query_plan_hash,
                                                              DATEADD(MINUTE,
                                                              DATEDIFF(MINUTE,
                                                              0, event_time),
                                                              0) ) AS logical_reads_50th ,
                        PERCENTILE_CONT(0.75) WITHIN GROUP ( ORDER BY logical_reads ) OVER ( PARTITION BY query_plan_hash,
                                                              DATEADD(MINUTE,
                                                              DATEDIFF(MINUTE,
                                                              0, event_time),
                                                              0) ) AS logical_reads_75th ,
                        PERCENTILE_CONT(0.90) WITHIN GROUP ( ORDER BY logical_reads ) OVER ( PARTITION BY query_plan_hash,
                                                              DATEADD(MINUTE,
                                                              DATEDIFF(MINUTE,
                                                              0, event_time),
                                                              0) ) AS logical_reads_90th ,
                        PERCENTILE_CONT(0.95) WITHIN GROUP ( ORDER BY logical_reads ) OVER ( PARTITION BY query_plan_hash,
                                                              DATEADD(MINUTE,
                                                              DATEDIFF(MINUTE,
                                                              0, event_time),
                                                              0) ) AS logical_reads_95th ,
                        PERCENTILE_CONT(0.99) WITHIN GROUP ( ORDER BY logical_reads ) OVER ( PARTITION BY query_plan_hash,
                                                              DATEADD(MINUTE,
                                                              DATEDIFF(MINUTE,
                                                              0, event_time),
                                                              0) ) AS logical_reads_99th ,
                        PERCENTILE_CONT(0.50) WITHIN GROUP ( ORDER BY writes ) OVER ( PARTITION BY query_plan_hash,
                                                              DATEADD(MINUTE,
                                                              DATEDIFF(MINUTE,
                                                              0, event_time),
                                                              0) ) AS writes_50th ,
                        PERCENTILE_CONT(0.75) WITHIN GROUP ( ORDER BY writes ) OVER ( PARTITION BY query_plan_hash,
                                                              DATEADD(MINUTE,
                                                              DATEDIFF(MINUTE,
                                                              0, event_time),
                                                              0) ) AS writes_75th ,
                        PERCENTILE_CONT(0.90) WITHIN GROUP ( ORDER BY writes ) OVER ( PARTITION BY query_plan_hash,
                                                              DATEADD(MINUTE,
                                                              DATEDIFF(MINUTE,
                                                              0, event_time),
                                                              0) ) AS writes_90th ,
                        PERCENTILE_CONT(0.95) WITHIN GROUP ( ORDER BY writes ) OVER ( PARTITION BY query_plan_hash,
                                                              DATEADD(MINUTE,
                                                              DATEDIFF(MINUTE,
                                                              0, event_time),
                                                              0) ) AS writes_95th ,
                        PERCENTILE_CONT(0.99) WITHIN GROUP ( ORDER BY writes ) OVER ( PARTITION BY query_plan_hash,
                                                              DATEADD(MINUTE,
                                                              DATEDIFF(MINUTE,
                                                              0, event_time),
                                                              0) ) AS writes_99th
        ...
        statement_analyzed_cte
          AS ( SELECT   sc.query_hash ,
                        sc.query_plan_hash ,
                        event_interval ,
                        sc.statement ,
               ...
               FROM     #query_stats sc
               GROUP BY sc.query_hash ,
                        sc.query_plan_hash ,
                        sc.statement ,
                        event_interval
             ),
        query_stats
          AS ( SELECT   sac.query_hash ,
                        sac.query_plan_hash ,
                        sac.statement ,
                        sac.event_interval ,
    ...
    SELECT  REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(LTRIM(RTRIM(qs.statement)),
                                                            NCHAR(13), ' '),
                                                    NCHAR(10), ' '), NCHAR(9),
                                            ' '), ' ', '<>'), '><', ''), '<>',
                    ' ') AS QueryText ,
    ...
    ORDER BY qs.event_interval ASC ,
...
WITH    waits_analyzed
        ...
        waits_ntile_cte
          AS ( SELECT  DISTINCT
                        ...
                        statement ,
                        wait_type ,
                        PERCENTILE_CONT(0.50) WITHIN GROUP ( ORDER BY duration_ms ) OVER ( PARTITION BY query_hash,
                                                              query_plan_hash,
                                                              event_interval,
                                                              statement,
                                                              wait_type ) AS duration_50th ,
                        PERCENTILE_CONT(0.75) WITHIN GROUP ( ORDER BY duration_ms ) OVER ( PARTITION BY query_hash,
                                                              query_plan_hash,
                                                              event_interval,
                                                              statement,
                                                              wait_type ) AS duration_75th ,
                        PERCENTILE_CONT(0.90) WITHIN GROUP ( ORDER BY duration_ms ) OVER ( PARTITION BY query_hash,
                                                              query_plan_hash,
                                                              event_interval,
                                                              statement,
                                                              wait_type ) AS duration_90th ,
                        PERCENTILE_CONT(0.95) WITHIN GROUP ( ORDER BY duration_ms ) OVER ( PARTITION BY query_hash,
                                                              query_plan_hash,
                                                              event_interval,
                                                              statement,
                                                              wait_type ) AS duration_95th ,
                        PERCENTILE_CONT(0.99) WITHIN GROUP ( ORDER BY duration_ms ) OVER ( PARTITION BY query_hash,
                                                              query_plan_hash,
                                                              event_interval,
                                                              statement,
                                                              wait_type ) AS duration_99th ,
                        PERCENTILE_CONT(0.50) WITHIN GROUP ( ORDER BY signal_duration_ms ) OVER ( PARTITION BY query_hash,
                                                              query_plan_hash,
                                                              event_interval,
                                                              statement,
                                                              wait_type ) AS signal_duration_50th ,
                        PERCENTILE_CONT(0.75) WITHIN GROUP ( ORDER BY signal_duration_ms ) OVER ( PARTITION BY query_hash,
                                                              query_plan_hash,
                                                              event_interval,
                                                              statement,
                                                              wait_type ) AS signal_duration_75th ,
                        PERCENTILE_CONT(0.90) WITHIN GROUP ( ORDER BY signal_duration_ms ) OVER ( PARTITION BY query_hash,
                                                              query_plan_hash,
                                                              event_interval,
                                                              statement,
                                                              wait_type ) AS signal_duration_90th ,
                        PERCENTILE_CONT(0.95) WITHIN GROUP ( ORDER BY signal_duration_ms ) OVER ( PARTITION BY query_hash,
                                                              query_plan_hash,
                                                              event_interval,
                                                              statement,
                                                              wait_type ) AS signal_duration_95th ,
                        PERCENTILE_CONT(0.99) WITHIN GROUP ( ORDER BY signal_duration_ms ) OVER ( PARTITION BY query_hash,
                                                              query_plan_hash,
                                                              event_interval,
                                                              statement,
                                                              wait_type ) AS signal_duration_99th
               FROM     #waits
             ),
        wait_rank
          AS ( SELECT   query_hash ,
    ...
    SELECT  REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(LTRIM(RTRIM(w.statement)),
                                                            NCHAR(13), ' '),
                                                    NCHAR(10), ' '), NCHAR(9),
                                            ' '), ' ', '<>'), '><', ''), '<>',
                    ' ') AS QueryText ,
