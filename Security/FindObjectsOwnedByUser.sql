...
(Select ' + CAST(database_id AS VARCHAR(9)) + ' as DBID, ''master'' as DBName, ' + REPLACE(@DB_Objects, '%D%', [name])
FROM    master.sys.databases
WHERE   [name] = 'master'

SELECT  @sql = @sql + 'UNION ALL Select ' + CAST(database_id AS VARCHAR(9)) + ', ''' + [name] + ''', ' + REPLACE(@DB_Objects, '%D%', [name])
FROM    master.sys.databases
WHERE   [name] != 'master'

SELECT  @sql = @sql + ') oo Where Login != ''' + @login + ''' ORDER BY type_desc, Login'
